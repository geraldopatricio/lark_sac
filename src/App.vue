...
const consultar = async () => {
  ...
  try {
    ...
    // Enviamos o valor do campo cliente para o parâmetro nfCnpj que o backend agora trata como híbrido
...
<template>
  <div class="min-h-screen p-2 md:p-8 bg-[#F8FAFC]">
      ...
      <div class="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-slate-100">
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead>
              <!-- Header com fontes menores no mobile -->
            ...
            <tbody class="divide-y divide-slate-100">
              <tr v-for="prod in paginatedProducts" :key="prod.PRODUTO_CODIGO" class="hover:bg-slate-50">
                ...
                <!-- CODFAB: Fonte menor no mobile -->
                <td class="px-3 md:px-8 py-4 text-slate-500 font-bold text-[10px] md:text-sm leading-tight">
                  {{ prod.PRODUTO_CODFAB }}
                </td>
                
                <!-- QTD: Badge compacta -->
                <td class="px-2 md:px-8 py-4 text-center">
                  <span class="bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md font-black text-[10px] md:text-xs">
                    {{ prod.PRODUTO_QUANTIDADE }}
                  </span>
                </td>

                <!-- VALOR ITEM: Fonte menor no mobile -->
                ...
                <!-- TOTAL ITEM: Fonte menor no mobile -->
            ...
            <tfoot class="bg-slate-900 text-white">
              <tr>
                <!-- Colspan ajustado para mobile (4 colunas visíveis) -->
...
<style>
/* Estilos para esconder setas padrão do select no Chrome/Safari */
select {
